SELECT region_id, land_area, eez_area, ts_area,
  ...
  (COALESCE(pa_marine_area,0) / marine_area * 100) AS percentage_pa_marine_cover,
  (COALESCE(well_connected_area,0) / (land_area + marine_area) * 100) AS percentage_well_connected,
  (COALESCE(importance_area,0) / (land_area + marine_area) * 100) AS percentage_importance,
  (COALESCE(pame_pa_land_area,0) / COALESCE(pa_land_area,0) * 100) AS pame_percentage_pa_land_cover,
  (COALESCE(pame_pa_marine_area,0) / COALESCE(pa_marine_area,0) * 100) AS pame_percentage_pa_marine_cover,
  CASE
...
FROM(
  ...
  sum(ts_area) ts_area,
  sum((land_area + marine_area) * percentage_well_connected / 100) well_connected_area,
  sum((land_area + marine_area) * percentage_importance / 100) importance_area,
